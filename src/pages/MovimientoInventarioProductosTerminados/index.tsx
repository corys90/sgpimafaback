...
const MovimientoInventarioProductosTerminados = () => {
    ...
    const listar = async () =>{
        const response = await httpApiGet("MovimientoInventarioProductosTerminados/GetAll");
    ...
    const OnbtnGuardar = async () => {
        ...
        if ((apiError.idCodigo && (apiError.idCodigo?.length > 0)) 
            ...
            if (prd.nombre === ""){
                ...
                const response = await httpApiPPPD("MovimientoInventarioProductosTerminados", "POST", {
    ...
    const buscaProducto = async () =>{
        ...
        if (response.statusCode >= 400){
            ...
            const um: any = emp.tipologia.unidadMedida.filter((itm: any)=>itm.id === response.data[0].unidadMedida);
            prd.um = (um.length > 0) ? um[0].nombre : "Sin definición";

            // trae la cantidad de stock del ineventario
            const responseInvPos = await httpApiGet(`InventarioProductosTerminados/GetByProductoId/${frmData.idCodigo}`);
        ...
        <div  className="container">
            ...
            <div className="container border rounded " style={{"color": "#2A3482"}}>
                ...
                <div className="d-flex">
                    <div className="h3 p-2 m-2  text-center text-wrap w-100">Ingreso de productos terminados</div>                                  
                </div>
...
export default MovimientoInventarioProductosTerminados;