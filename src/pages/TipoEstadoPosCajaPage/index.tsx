...
const TipoEstadoPosCajaPage = () => {
    
    const [estadosVisibles, setEstadosVisibles] = useState(false);
    const [tituloBoton, setTituloBoton] = useState("Mostrar tipos de estados del POS");
    ...
    const listar = async () =>{
        const response = await httpApiGet("PosTipoEstadoPosCaja");
    ...
    const verListado = async () => {
        ...
        setTituloBoton(!estadosVisibles ? "Ocultar tipos de estados del POS" : "Mostrar tipos de estados del POS");
    ...
    const OnbtnGuardar = async () => {
        ...
        if (((apiError.nombre) && (apiError.nombre?.length > 0)) || ((apiError.descripcion) && (apiError.descripcion?.length > 0))){
            ...
            if (btnRef === "Guardar"){
                ...
                const response = await httpApiPPPD("PosTipoEstadoPosCaja", "POST", {
                ...
                const response = await httpApiPPPD(`PosTipoEstadoPosCaja/${frmData.id}/${frmData.nombre}`, "PUT", {
                    "Content-Type" : "application/json"
                }, frmData);
                if (response.statusCode >= 400){
                    setOperacion(false);
            ...
            <div className=' vh-100 ms-5 me-5 border rounded-3 shadow'>
                ...
                <div  className=' d-flex justify-content-evenly align-items-center bg-body-tertiary'>
                    <div className="border p-1 rounded w-100" style={{"color": "#2A3482"}}>
                        <a id="inicio"></a>
                        <label htmlFor="" className="h3 p-2 m-2">Estados del P.O.S.</label>
                    ...
                    { showYesNo && <MsgYesNoDialog
                            Title='Tipos de estados del POS'
...
export default TipoEstadoPosCajaPage;