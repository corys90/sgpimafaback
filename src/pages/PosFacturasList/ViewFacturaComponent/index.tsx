...
const ViewFacturaComponent = (props:{ Factura: number}) => {
    ...
    const columnas = [      
        ...
        {
            name: 'U.M.',
            selector: (row: FormDetalleFactura) => row.unidadMedida,
            width: "80px",
            wrap: true,
            sortable: true,          
        },         
    ...
    const Vendedor = (props: {idVendedor: number, nombres: string, onClickCallback: any} ) =>{
        return(
            <div>
                <input type="radio" id={`vnd-${props.idVendedor}`} name={`vnd-vendedores`} className=" btn-check"  disabled
                    autoComplete="off" onChange={props.onClickCallback} checked={true}                    />
    ...
    useEffect(()=>{
        ...
        const getVendedor = async (id: number)=>{
            const response = await httpApiGet(`PosVendedor/${id}`);
            if (response.statusCode >= 400){
                mensajeModal = [...response.messages];
                setMensajeModal(mensajeModal);            
            }else{
                vendedor = response.data[0].nombres;
                setVendedor(vendedor);            
            }
        } 

        const buscaCliente = async (tc: any, nit: any) =>{
            const response = await httpApiGet(`Cliente/GetByIdentification/${tc}/${nit}`);
            ...
            if (response.statusCode >= 400){
                ...
                const dataClient = {
                    razonSocial : response.data[0].nombres + " " + response.data[0].apellidos,
        ...
        const getFactura = async (id: number)=>{
            const response = await httpApiGet(`Posfactura/${id}`);
            ...
            if (response.statusCode >= 400){
                ...
                frmData = response.data[0];
                const clnt = await buscaCliente(frmData.tipoCliente, frmData.nit);
                getVendedor(frmData.idVendedor);
                frmData.dpto = clnt?.dpto;
                frmData.ciudad = clnt?.ciudad;
                frmData.direccion = clnt?.direccion;
                frmData.razonSocial = "" + clnt?.razonSocial;
                frmData.telefono = clnt?.telefono
                frmData.fechaFactura = frmData.fechaFactura?.substring(0, 10);
                frmData.fechaVencimiento = frmData.fechaVencimiento?.substring(0, 10);
                setFormData({...frmData});
            }
        }       

        const getDetalles = async (id: number)=>{
            const response = await httpApiGet(`PosPosfacturaDetalle/GetByFactura/${id}`);
        ...
        getTipoIdPos();
        getTipoDocumentos();   
        getFactura(parseInt(`${props.Factura}`));    
        getDetalles(parseInt(`${props.Factura}`));    
             ...
             <div className="col-lg-12 col-md-12  col-sm-12 ">
                <div className=" border p-1 rounded" style={{"color": "#2A3482"}}>
                    ...
                    <div className=" row p-2">
                        ...
                        <div  className=" col-lg-4 col-md-12 ">
                            <div className=" col-lg-4 col-md-12 text-center border rounded p-4 w-100">
                                ...
                                <p className="h3 fw-bolder border p-2">{props.Factura.toString().padStart(7, '0')}</p>   
                    ...
                    <form className='row border rounded p-2 m-2'>
                        ...
                        <div className="col-lg-6 col-md-12 col-sm-12 mb-3">
                            <label htmlFor="concepto" className="form-label">Por Concepto De</label>                  
                        ...
                        <div className="col-lg-12 col-md-12 col-sm-12 mb-3">
                            <div className="btn-group d-flex justify-content-center flex-wrap gap-4" role="group" aria-label="Basic radio toggle button group">
                                <Vendedor idVendedor={frmData.idVendedor} nombres={vendedor} onClickCallback={null} />
                            </div>
                        </div>    
                    ...
                    <div className="row d-flex">  
                        <div className="col-lg-4 col-md-12 col-sm-12">
                            <Button className="m-1 p-2 btn-success w-100 p-4" id="btnImprimir" disabled >PDF/Imprimir</Button>   
                        </div>                         
                    </div>    
                    {/*                     */}  
                    { showMsg && <MsgDialog
