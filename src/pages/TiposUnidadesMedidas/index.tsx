...
const TiposUnidadesMedidas = () => {
    ...
    const verListado = async () => {
        ...
        setTituloBoton(!estadosVisibles ? "Ocultar las unidades de medidas" : "Mostrar las unidades de medidas");
    ...
    const OnbtnGuardar = async () => {
        ...
        if (((apiError.nombre) && (apiError.nombre?.length > 0)) || ((apiError.descripcion) && (apiError.descripcion?.length > 0))){
            ...
            if (btnRef === "Guardar"){
                ...
                const response = await httpApiPPPD("PosUnidadesMedida", "POST", {
                ...
                const response = await httpApiPPPD(`PosUnidadesMedida/${frmData.id}/${frmData.nombre}`, "PUT", {
                    "Content-Type" : "application/json"
                }, frmData);
                if (response.statusCode >= 400){
                    setOperacion(false);
            ...
            <div className=' vh-100 ms-5 me-5 border rounded-3 shadow'>
                ...
                <div  className='d-flex justify-content-evenly align-items-center bg-body-tertiary'>
                    <div className="border p-1 rounded w-100" style={{"color": "#2A3482"}}>
                        ...
                        <form className='row border p-2 m-2'>
                            ...
                            <div className="col-lg-8 col-md-12 col-sm-12 mb-3">
                                <label htmlFor="direccion" className="form-label">Descripción de la unidad</label>                  
                    ...
                    { showYesNo && <MsgYesNoDialog
                            Title='Tipo de unidades de medidas'
                            Message={`Este proceso cambia el estado de la unidad de medida, lo deshabilita si está habilitado o visceversa. ¿Quiero cambiarlo?`}
                            Icon='X'
