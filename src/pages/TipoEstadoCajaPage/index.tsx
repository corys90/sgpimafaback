...
const TipoEstadoCajaPage = () => {
    
    const [estadosVisibles, setEstadosVisibles] = useState(false);
    const [tituloBoton, setTituloBoton] = useState("Mostrar tipos de estados de la caja");
    ...
    const listar = async () =>{
        const response = await httpApiGet("PosTipoEstadoCaja");
    ...
    const verListado = async () => {
        ...
        setTituloBoton(!estadosVisibles ? "Ocultar tipos de estados de la caja" : "Mostrar tipos de estados de la caja");
    ...
    const OnbtnGuardar = async () => {
        ...
        if (((apiError.nombre) && (apiError.nombre?.length > 0)) || ((apiError.descripcion) && (apiError.descripcion?.length > 0))){
            ...
            if (btnRef === "Guardar"){
                ...
                const response = await httpApiPPPD("PosTipoEstadoCaja", "POST", {
                ...
                if (response.statusCode >= 400){
                    ...
                    msg = "Se ha creado el nuevo tipo de estado exitosamente!!!"
                ...
                const response = await httpApiPPPD(`PosTipoEstadoCaja/${frmData.id}/${frmData.nombre}`, "PUT", {
                    "Content-Type" : "application/json"
                }, frmData);
                if (response.statusCode >= 400){
                    setOperacion(false);
                    ...
                    msg = "Se ha actualizado el tipo de estado exitosamente!!!"
                    mensajeModal.push(msg);
                    OnbtnLimpiar();
                }  
            }
            ...
            <div className=' vh-100 ms-5 me-5 border rounded-3 shadow'>
                ...
                <div  className=' d-flex justify-content-evenly align-items-center bg-body-tertiary'>
                    <div className="border p-1 rounded w-100" style={{"color": "#2A3482"}}>
                        <a id="inicio"></a>
                        <label htmlFor="" className="h3 p-2 m-2">Estados de caja</label>
                        <form className='row border p-2 m-2'>
                            ...
                            <div className="col-lg-8 col-md-12 col-sm-12 mb-3">
                                <label htmlFor="direccion" className="form-label">Descripción del estado</label>                  
                    ...
                    { showYesNo && <MsgYesNoDialog
                            ...
                            Message={`Este proceso cambia el estado de la caja, lo deshabilita si está habilitado o visceversa. ¿Quiero cambiarlo?`}
                            Icon='X'
...
export default TipoEstadoCajaPage;